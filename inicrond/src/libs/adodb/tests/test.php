...
FROM `nuke_stories` `t1`, `nuke_authors` `t2`, `nuke_stories_cat` `t3`, `nuke_topics` `t4` 
	...
	if ($rs->EOF && (($ox = $rs->FetchField(0)) && !empty($ox->name))) {
		...
		$query =  $db->GetInsertSQL($rs, $record);
...
*/
	...
	case 'oci8po':
		...
		*/
		...
		$query = "INSERT INTO photos ( ID, photo) ".
...
*/
	...
	$query = "insert into ADOXYZ (id,firstname,lastname) values (".$db->Param('0').",".$db->Param('1').",".$db->Param('2').")";
	$db->StartTrans();
	$db->Execute($sql,$arr);
	$db->CompleteTrans();
	$rs = $db->Execute('select * from ADOXYZ order by id');
  ...
  PRIMARY KEY  (row1))";
		...
		$query = "insert into ADOXYZ (id,firstname,lastname,created) values ($i*10+0,?,?,$time)";
		break;
	case 'mysqli':
	case 'mysqlt':
	case 'mysql':
  ...
  KEY `myindex` (`row1`,`row2`)
	...
	case 'oci805':
		...
		$amt = rand() % 100;
		$query = "insert into ADOXYZ (id,firstname,lastname,created) values ($i*10+0,:first,:last,$time)";		
		break;
	}
	if ($i & 1) {
 ...
 //	$db->debug=true;
	...
	if ($db->dataProvider == 'postgres') {
		$query = "select ".$db->Concat('cast(firstname as varchar)',$db->qstr(' '),'lastname')." as fullname,id,".$db->sysTimeStamp." as d from ADOXYZ";
		$rs = &$db->Execute($sql);
	} else {
		$query = "select distinct ".$db->Concat('firstname',$db->qstr(' '),'lastname')." as fullname,id,".$db->sysTimeStamp." as d from ADOXYZ";
		$rs = &$db->Execute($sql);
	}
	if ($rs) {
		if (empty($_GET['hide'])) rs2html($rs);
	...
	$query = 'select * from ADOXYZ where 0=1';
	...
	if (substr($db->dataProvider,0,3) != 'notused') { // used to crash ado
		$query = "select firstnames from adoxyz";
