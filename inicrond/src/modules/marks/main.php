...

{
	...
	AND
	".$_OPTIONS['table_prefix'].$_OPTIONS['tables']['usrs'].".usr_id=".$_OPTIONS['table_prefix'].$_OPTIONS['tables']['scores'].".usr_id
	AND
	time_stamp_end>time_stamp_start
	AND
	...
	)
	{
		if(isset($_GET['cours_id']) AND
                ...
                $module_content .= "<a href=\"../../modules/marks/time_vs_score_img.php?usr_id=".$_GET['usr_id']."\" >".$_LANG['correlation_between_time_and_score']."</a><br />";
                
                
                
                $module_content .= "<a href=\"../../modules/marks/normal_dist_img.php?usr_id=".$_GET['usr_id']."\" >".$_LANG['GD_distribution_of_score']."</a><br />";
                
                
                
                $module_content .= "<a href=\"../../modules/marks/normal_dist_time_img.php?usr_id=".$_GET['usr_id']."\" >".$_LANG['distribution_of_time']."</a><br />";
                ...
                //$img = "graph.php?usr_id=".$_GET['usr_id'];//image
                ...
                {	
                        ...
                        WHERE
                        ".$_OPTIONS['table_prefix'].$_OPTIONS['tables']['scores'].".usr_id=".$_GET['usr_id']."
                        AND
                        ".$_OPTIONS['table_prefix'].$_OPTIONS['tables']['chapitre_media'].".chapitre_media_id=".$_OPTIONS['table_prefix'].$_OPTIONS['tables']['scores'].".chapitre_media_id
                        AND
                        time_stamp_end>time_stamp_start
...
is_in_charge_of_user($_SESSION['usr_id'], session_id_to_usr($_GET['session_id']))
	...
	elseif(
	isset($_GET['chapitre_media_id']) AND
	...
	)
	{
                ...
                AND
                ".$_OPTIONS['table_prefix'].$_OPTIONS['tables']['scores'].".usr_id=".$_OPTIONS['table_prefix'].$_OPTIONS['tables']['groups_usrs'].".usr_id
                AND
                ".$_OPTIONS['table_prefix'].$_OPTIONS['tables']['chapitre_media'].".chapitre_media_id=".$_OPTIONS['table_prefix'].$_OPTIONS['tables']['scores'].".chapitre_media_id
                ";
                ...
                $rs = $inicrond_db->Execute($query);
                while(!$rs->EOF AND $f = $rs->FetchRow())
                {
                        if(!isset($already_there[$f['chapitre_media_id']]))
                        {
	...
	elseif(isset($_GET['start']) AND//par date
        isset($_GET["end"]) AND
        is_numeric($_GET['cours_id']) AND
        is_in_charge_in_course($_SESSION['usr_id'], $_GET['cours_id']) //AND
        //!isset($_GET[""])
        )//par date
        {
                $it_is_ok = TRUE;
	...
	'points_obtenu/points_max*100' => array(
        ...
        elseif(isset($_GET['chapitre_media_id']) AND
        !isset($_GET['join'])
        )
        {
