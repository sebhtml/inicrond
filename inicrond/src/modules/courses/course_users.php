...
if (isset ($_SESSION['usr_id']) &&
    ...
    is_in_charge_in_course ($_SESSION['usr_id'], $_GET['cours_id']))
  {
	...
	$course["admin_grps"] = retournerHref ("grps_for_ensei.b.php?cours_id=".$_GET['cours_id'], $_LANG['modify']);	//groupes -> cours
	...
	$big_array['administrators'][$i]['group_name'] =
	  $fetch_result['group_name'];
	$big_array['administrators'][$i]["group_link"] =
	  "../../modules/groups/grp.php?group_id=".$fetch_result['group_id'];

	$big_array['administrators'][$i]["edit_link"] =
	  "<a href=\"".__INICROND_INCLUDE_PATH__.
	  "modules/groups/edit_a_group.php?group_id=".
	...
	while ($fetch_result = $rs2->FetchRow ())
	  {
	    $big_array['administrators'][$i]["users"][$j]['usr_name'] =
	      $fetch_result['usr_name'];
	    $big_array['administrators'][$i]["users"][$j]['usr_nom'] =
	      $fetch_result['usr_nom'];
	    $big_array['administrators'][$i]["users"][$j]['usr_prenom'] =
	      $fetch_result['usr_prenom'];
	    $big_array['administrators'][$i]["users"][$j]["usr_link"] =
	      __INICROND_INCLUDE_PATH__."modules/members/one.php?usr_id=".
	...
	if ($CAN_ADMIN_STUDENTS_GRPS)
	  {
	    $big_array['students'][$i]["change_charge_link"] =
	      __INICROND_INCLUDE_PATH__.
	      "modules/courses/groups_in_charge_for_course_group.php?group_id=".
	      $fetch_result['group_id'];


	  }

	$big_array['students'][$i]['group_name'] =
	  "<a href=".__INICROND_INCLUDE_PATH__.
	  "modules/groups/grp.php?group_id=".$fetch_result['group_id'].">".
	  $fetch_result['group_name']."</a>";
	$big_array['students'][$i]["edit_group_link"] =
	  "<a href=".__INICROND_INCLUDE_PATH__.
	  "modules/groups/edit_a_group.php?group_id=".
	  $fetch_result['group_id'].">".$_LANG['edit']."</a>";
	...
	while (!$rs2->EOF AND $fetch_result = $rs2->FetchRow ())
	  {
	    $count++;
	    $big_array['students'][$i]["users"][$j]['usr_name'] =
	      $fetch_result['usr_name'];
	    $big_array['students'][$i]["users"][$j]['usr_nom'] =
	      $fetch_result['usr_nom'];
	    $big_array['students'][$i]["users"][$j]['usr_prenom'] =
	      $fetch_result['usr_prenom'];
	    $big_array['students'][$i]["users"][$j]["usr_link"] =
	      __INICROND_INCLUDE_PATH__."modules/members/one.php?usr_id=".
	      $fetch_result['usr_id']."&cours_id=".$_GET['cours_id'];
	    $j++;
	  }
	...
	while (!$rs3->EOF AND $fetch_result = $rs3->FetchRow ())
	  {
	    ...
	    $big_array['students'][$i]['groups_in_charge'][$k]['group_name'] =
	      ...
	      "modules/groups/grp.php?group_id=".$fetch_result['group_id'].
	      ">".$fetch_result['group_name']."</a>";
	    $big_array['students'][$i]['groups_in_charge'][$k]
	      ["edit_group_link"] =
	      "<a href=".__INICROND_INCLUDE_PATH__.
	      "modules/groups/edit_a_group.php?group_id=".
	      $fetch_result['group_id'].">".$_LANG['edit']."</a>";
	    $big_array['students'][$i]['groups_in_charge'][$k]["users"] =
	      array ();
	    $j = 0;
	    $query = "SELECT 
                        usr_name,
	    ...
	    while (!$rs2->EOF AND $fetch_result = $rs2->FetchRow ())
	      {
		$big_array['students'][$i]['groups_in_charge'][$k]["users"]
		  [$j]['usr_name'] = $fetch_result['usr_name'];
		$big_array['students'][$i]['groups_in_charge'][$k]["users"]
		  [$j]['usr_nom'] = $fetch_result['usr_nom'];
		$big_array['students'][$i]['groups_in_charge'][$k]["users"]
		  [$j]['usr_prenom'] = $fetch_result['usr_prenom'];
		$big_array['students'][$i]['groups_in_charge'][$k]["users"]
		  [$j]["usr_link"] =
		  __INICROND_INCLUDE_PATH__."modules/members/one.php?usr_id=".
		  $fetch_result['usr_id']."&cours_id=".$_GET['cours_id'];
