...
if (isset ($_GET['inode_id_location']) AND
    $_GET['inode_id_location'] != "" AND
    (int) $_GET['inode_id_location'] AND
    $_GET['inode_id_location'] != 0 AND
...
elseif (isset ($_GET['cours_id']) AND
	$_GET['cours_id'] != "" AND
	(int) $_GET['cours_id'] AND is_numeric ($_GET['cours_id']))
{
...
if ($ok AND
    $is_student_of_cours =
    is_student_of_cours ($_SESSION['usr_id'], $cours_id))
  {



    $is_in_charge_in_course =
    ...
    if ($_OPTIONS['smarty_cache_config']['courses']["inode.tpl"])	// != 0
      {
	$smarty->caching = 1;
    ...
    $cache_id = md5 ($_SESSION['language'].
		     $cours_id.
		     $is_teacher_of_cours.
		     $_SESSION['SUID'].$inode_id_location);
	...
	if (isset ($_GET['inode_id_location']) AND
	    $_GET['inode_id_location'] != 0)
	  {
	    ...
	    if ($fetch_result['inode_id_location'])	// != 0
	      {
		$smarty->assign ("parent_dir",
				 __INICROND_INCLUDE_PATH__.
				 "modules/courses/inode.php?&inode_id_location=".
				 $fetch_result['inode_id_location']);
	      }
	    else
	      {
		$smarty->assign ("parent_dir",
	...
	else			//all the link for this course hehe.
	  {
	    ...
	    $course["forums_link"] =
	      __INICROND_INCLUDE_PATH__.
	      "modules/mod_forum/main_inc.php?&cours_id=".$_GET['cours_id'];





	    $course['cours_description'] =
	      BBcode_parser ($fetch_result['cours_description']);
		...
		$course['see_online_people_for_a_course'] =
		  retournerHref (__INICROND_INCLUDE_PATH__.
				 "modules/seSSi/see_online_people_for_a_course.php?cours_id=".
				 $_GET['cours_id']."",
				 $_LANG['see_online_people_for_a_course']);
		$course['inicrond_x_module'] =
		  "<a href=\"".__INICROND_INCLUDE_PATH__.
		  "modules/inicrond_x/inicrond_x_groups_selection.php?&cours_id=$cours_id\">".
		  $_LANG['inicrond_x_module']."</a>";
	...
	$inode_id_location = (isset ($_GET['inode_id_location']) AND	//check the requested inode.
			      $_GET['inode_id_location'] != "" AND
	...
	if ($is_teacher_of_cours)
	  {
	    ...
	    $smarty->assign ('add_dir',
			     __INICROND_INCLUDE_PATH__.
			     "modules/courses/add_dir.php?inode_id_location=".
			     $inode_id_location."&cours_id=".$cours_id."");
	...
	while ($fetch_result = $rs->FetchRow ())
	  {
		...
		$dir['edit'] =
		  __INICROND_INCLUDE_PATH__.
		  "modules/courses/edit_dir.php?inode_id=".
		  $fetch_result['inode_id']."";
	      }
		...
		$dir['inode_up'] =
		  __INICROND_INCLUDE_PATH__.
		  "modules/courses/inode_up.php?inode_id=".
		  $fetch_result['inode_id']."";
	      }
	    if ($can_down)
	      {
		$dir['inode_down'] =
		  __INICROND_INCLUDE_PATH__.
		  "modules/courses/inode_down.php?inode_id=".
		  $fetch_result['inode_id']."";
	      }
	...
	if ($is_teacher_of_cours)
	  {
	    ...
	    $smarty->assign ('add_file',
			     "../../modules/files_4_courses/add_edit_file.modu.php?inode_id_location=$inode_id_location&cours_id=$cours_id");
	...
	while ($fetch_result = $rs->FetchRow ())
	  {
	    ...
	    $file["my_results_link"] =
	      '<a href="'.__INICROND_INCLUDE_PATH__.
	      "modules/dl_acts_4_courses/show_dl_acts.mo.php?file_id=".
	      $fetch_result['file_id']."&usr_id=".$_SESSION['usr_id']."&join".
	      '">'.$_LANG['dl_acts_4_courses'].'</a>';

	    $file['inode_id'] = $fetch_result['inode_id'];
	    $file["link"] =
	      retournerHref (__INICROND_INCLUDE_PATH__.
			     "modules/files_4_courses/download.php?file_id=".
			     $fetch_result['file_id']."",
			     $fetch_result['file_title']);
		...
		$file['edit'] =
		  __INICROND_INCLUDE_PATH__.
		  "modules/files_4_courses/add_edit_file.modu.php?file_id=".
		  $fetch_result['file_id']."";
	      }
		...
		$file["dl_report"] =
		  __INICROND_INCLUDE_PATH__.
		  "modules/dl_acts_4_courses/group_downloads_reporting.php?file_id=".
		  $fetch_result['file_id']."";
	...
	if ($is_teacher_of_cours)
	  {
	    ...
	    $smarty->assign ('add_test',
			     __INICROND_INCLUDE_PATH__.
			     "modules/tests-php-mysql/add_test.php?inode_id_location=".
			     $inode_id_location."&cours_id=".$cours_id."");
	...
	while ($fetch_result = $rs->FetchRow ())
	  {
	    ...
	    $test["my_results_link"] =
	      __INICROND_INCLUDE_PATH__.
	      "modules/tests-results/results.php?usr_id=".$_SESSION['usr_id'].
	      "&test_id=".$fetch_result['test_id']."&join";
	    $test["link"] =
	      retournerHref (__INICROND_INCLUDE_PATH__.
			     "modules/tests-php-mysql/do_it.php?test_id=".
			     $fetch_result['test_id']."",
			     $fetch_result['test_name']);
	    $test["time_GMT_add"] =
	      format_time_stamp ($fetch_result["time_GMT_add"]);
	    $test["time_GMT_edit"] =
	      format_time_stamp ($fetch_result["time_GMT_edit"]);
		...
		$test['edit'] =
		  __INICROND_INCLUDE_PATH__.
		  "modules/tests-php-mysql/edit.php?test_id=".
		  $fetch_result['test_id'];

		$test['edit_a_test_GOLD'] =
		  __INICROND_INCLUDE_PATH__.
		  "modules/tests-php-mysql/edit_a_test_GOLD.php?test_id=".
		  $fetch_result['test_id'];
		...
		$test["mass_update"] =
		  __INICROND_INCLUDE_PATH__.
		  "modules/tests-php-mysql/update_test_results.php?test_id=".
		  $fetch_result['test_id']."";
		...
		$test['scores'] =
		  __INICROND_INCLUDE_PATH__.
		  "modules/tests-results/results.php?test_id=".
		  $fetch_result['test_id']."";
		$test["report"] =
		  __INICROND_INCLUDE_PATH__.
		  "modules/tests-results/test_activities_report.php?&test_id=".
		  $fetch_result['test_id'];
	...
	if ($is_teacher_of_cours)
	  {
	    ...
	    $smarty->assign ('add_swf',
			     __INICROND_INCLUDE_PATH__.
			     "modules/course_media/add_flash.php?inode_id_location=".
			     $inode_id_location."&cours_id=".$cours_id);

	    $smarty->assign ('add_img',
			     __INICROND_INCLUDE_PATH__.
			     "modules/course_media/add_img.php?inode_id_location=".
			     $inode_id_location."&cours_id=".$cours_id);

	    $smarty->assign ('add_text',
			     __INICROND_INCLUDE_PATH__.
			     "modules/course_media/add_text.php?inode_id_location=".
			     $inode_id_location."&cours_id=".$cours_id);
	...
	while ($fetch_result = $rs->FetchRow ())
	  {
	    ...
	    $anim["my_results_link"] =
	      __INICROND_INCLUDE_PATH__."modules/marks/main.php?usr_id=".
	      $_SESSION['usr_id']."&chapitre_media_id=".
	      $fetch_result['chapitre_media_id']."&join";

	    $anim['inode_id'] = $fetch_result['inode_id'];

	    $anim['file_name'] = $fetch_result['file_name'];
	    $anim['chapitre_media_edit_gmt_timestamp'] =
	      format_time_stamp ($fetch_result
				 ['chapitre_media_edit_gmt_timestamp']);
	    $anim['chapitre_media_description'] =
	      BBcode_parser ($fetch_result['chapitre_media_description']);
	    $anim['chapitre_media_add_gmt_timestamp'] =
	      format_time_stamp ($fetch_result
				 ['chapitre_media_add_gmt_timestamp']);

	    $anim["link"] =
	      retournerHref
	      ("javascript:popup('../../modules/course_media/flash.php?chapitre_media_id=".
	       $fetch_result['chapitre_media_id']."&question_ordering_id=".
	       $question_ordering_id."', 790, 590)",
		...
		$anim['edit'] =
		  __INICROND_INCLUDE_PATH__.
		  "modules/course_media/edit_flash.php?chapitre_media_id=".
		  $fetch_result['chapitre_media_id'];
		...
		$anim['marks'] =
		  __INICROND_INCLUDE_PATH__.
		  "modules/marks/main.php?chapitre_media_id=".
		  $fetch_result['chapitre_media_id']."";
		$anim["report"] =
		  __INICROND_INCLUDE_PATH__.
		  "modules/marks/flash_activities_report.php?&chapitre_media_id=".
		  $fetch_result['chapitre_media_id'];
	      }
	...
	while ($fetch_result = $rs->FetchRow ())
	  {
	    ...
	    $anim["img_url"] =
	      __INICROND_INCLUDE_PATH__.
	      "modules/course_media/download_img.php?img_id=".
	      $fetch_result["img_id"];
	    ...
	    $anim['chapitre_media_edit_gmt_timestamp'] =
	      format_time_stamp ($fetch_result["edit_time_t"]);
	    $anim['chapitre_media_description'] =
	      nl2br ($fetch_result['img_description']);
	    $anim['chapitre_media_add_gmt_timestamp'] =
	      format_time_stamp ($fetch_result["add_time_t"]);

	    /*$anim["link"] = retournerHref("javascript:popup('../../modules/courses/flash.php?chapitre_media_id=".$fetch_result['chapitre_media_id']."&.', 790, 590)",
	       $fetch_result['chapitre_media_title']); */
		...
		$anim['edit'] =
		  __INICROND_INCLUDE_PATH__.
		  "modules/course_media/edit_image.php?img_id=".
		  $fetch_result["img_id"];
	      }
	    if ($can_drop)
	      {
	...
	if ($is_teacher_of_cours)	//the teacher query.
	  {
	    ...
	    $query = "SELECT
                        ...
                        content_type='5'
                        AND
                        ".$_OPTIONS['table_prefix'].$_OPTIONS['tables']['inode_elements'].".content_id=".$_OPTIONS['table_prefix'].$_OPTIONS['tables']['inicrond_texts'].".text_id
	...
	while ($fetch_result = $rs->FetchRow ())
	  {
	    ...
	    $anim['chapitre_media_edit_gmt_timestamp'] =
	      format_time_stamp ($fetch_result["edit_time_t"]);
	    $anim['chapitre_media_description'] =
	      nl2br ($fetch_result['text_description']);
	    $anim['chapitre_media_add_gmt_timestamp'] =
	      format_time_stamp ($fetch_result["add_time_t"]);



	    /*$anim["link"] = retournerHref("javascript:popup('../../modules/courses/flash.php?chapitre_media_id=".$fetch_result['chapitre_media_id']."&question_ordering_id=".$question_ordering_id."', 790, 590)",
	       $fetch_result['chapitre_media_title']); */
		...
		$anim['edit'] =
		  __INICROND_INCLUDE_PATH__.
		  "modules/course_media/edit_text.php?text_id=".
		  $fetch_result["text_id"];
	      }
	    if ($can_drop)
	      {
		$anim['drop'] =
	...
	//echo nl2br(print_r($dirs_list,true)); exit();
	...
	if (!$_GET['inode_id_location'])	//show those thing only for inode 0
	  {
	    ...
	    if ($_SESSION['SUID'])
	      {
		$course['drop'] = retournerHref (__INICROND_INCLUDE_PATH__."modules/courses/remove_course.php?cours_id=".$_GET['cours_id']."", $_LANG['remove']);	//enlever
		...
		$course['edit'] =
		  retournerHref (__INICROND_INCLUDE_PATH__.
				 "modules/courses/add_edit_course.php?cours_id=".
				 $_GET['cours_id'], $_LANG['edit']);



		$smarty->assign ('course_groups_listing',
				 "<a href=\"".__INICROND_INCLUDE_PATH__.
				 "modules/groups/course_groups_listing.php?cours_id=".
				 $cours_id."\">".
	...
	//get the cours variable.

	if ($is_teacher_of_cours)
	  {
	    $smarty->assign ('course_admin_menu',
			     "<a href=\"../../modules/course_admin/course_admin_menu.php?cours_id=$cours_id\">".
	...
	$smarty->assign ('blue_master_clone',
			 "<a href=\"".__INICROND_INCLUDE_PATH__.
			 "modules/blue_master_clone/blue_master_clone.php?cours_id=".
			 $_GET['cours_id']."\">".$_LANG['blue_master_clone'].
			 "</a>");
	...
	if (is_in_charge_in_course ($_SESSION['usr_id'], $cours_id))
	  {
	    $smarty->assign ('course_users',
			     retournerHref (__INICROND_INCLUDE_PATH__.
					    "modules/courses/course_users.php?cours_id=".
					    $_GET['cours_id']."",
					    $_LANG['course_users']));
	...
	$smarty->assign ('calendar',
			 "<a href=\"".__INICROND_INCLUDE_PATH__.
			 "modules/calendar/main_inc.php?&cours_id=$cours_id\">".
			 $_LANG['calendar']."</a>");
