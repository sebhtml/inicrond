...
include __INICROND_INCLUDE_PATH__.
  "modules/groups/includes/functions/group_id_to_cours_id.php";
...
if (isset ($_GET['group_id']) &&
    ...
    is_teacher_of_cours ($_SESSION['usr_id'], $cours_id))
  {
	...
	$checked =
	  isset ($fetch_result2["group_in_charge_group_id"]) ? "CHECKED" : "";

	$tab[] =
	  array ($fetch_result['group_name'],
		 " <input $checked type=\"checkbox\" name=\"group_in_charge_group_id=".
		 $fetch_result['group_id']."\" value=\"\" />");
      }

    $module_content .=  retournerTableauXY ($tab);

    $module_content .=  "<input  type=\"submit\" name=\"data_sent\" /> </form>
        <h2><a href=\"course_users.php?&cours_id=$cours_id\">".$_LANG['course_users']."</a></h2>";
