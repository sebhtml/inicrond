...
class Loi_normal
{
	...
	var $nb_data;//nombre de données.
	
	var $standard_dev;//écart type corrigé.
	...
	echo "distrib";
	echo nl2br(print_r( $this->distrib, TRUE));//données.
	echo "<br />";
	...
	echo "nb_data";
	echo $this->nb_data;//nombre de données.
	echo "<br />";
	
	echo "standard_dev";
	echo $this->standard_dev;//écart type corrigé.
	echo "<br />";
	...
	$i= 0 ;
		...
		$count = 0;
			...
			//echo $key.": <br />";
				...
				if($nb_class == $this->nb_class AND//la première classe
				$value < $distrib[$i]["z"]["min"] //AND
				//$value <= $distrib[$i]["z"]["max"])//la dernière, important...
				)
				{
					if($this->debug)
					{
				...
				elseif($value >= $distrib[$i]["z"]["min"] AND//les classes intermédiaires.
				...
				elseif($nb_class == 0 AND//la dernière classe
				$value >= $distrib[$i]["z"]["min"] //AND
				//$value <= $distrib[$i]["z"]["max"])//la dernière, important...
				)
				{
