<?php
/*
    $Id: view_evaluations_with_a_group.php 87 2006-01-01 02:20:14Z sebhtml $
...
)
{
    ...
    include PEAR_PATH.'Math/Stats.php';//PEAR MATH STATS BY Jesus (seriously!!)
    ...
    while($fetch_result = $rs->FetchRow())
    {
        if(!$fetch_result['available'] && isset($_GET['usr_id']))
        {
            continue;
        }
    ...
    while($fetch_result = $rs->FetchRow())
    {
        ...
        while($fetch_result = $rs2->FetchRow())//foreach evaluation I got.
        {
            ...
            if(isset($fetch_result['ev_max']) &&
            $fetch_result['ev_max'] &&
            is_numeric($evaluations_stats[$fetch_result['ev_id']]['stdev']))//not eq 0.
            {

                //print_r($evaluations_stats["18"]);
                if(isset($evaluations_stats[$fetch_result['ev_id']]['stdev']) &&
                $evaluations_stats[$fetch_result['ev_id']]['stdev'] &&
                isset($fetch_result['ev_id']) &&
                $fetch_result['ev_id'] != '')
                {
                    $value =
        ...
        if($final_marks_dataset_stats['stdev'] &&
        $final_mark[$i]['points_max'] &&
