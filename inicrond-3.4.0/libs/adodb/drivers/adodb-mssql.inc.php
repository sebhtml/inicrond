...
"select c.name,t.name,c.length,
	...
	*/
	...
	function &MetaIndexes($table,$primary=false)
	{
		...
		$query = "SELECT i.name AS ind_name, C.name AS col_name, USER_NAME(O.uid) AS Owner, c.colid, k.Keyno, 
...
order by constraint_name, referenced_table_name, keyno";
	...
	global $ADODB_FETCH_MODE;
		...
		$query = "select distinct k.column_name,ordinal_position from information_schema.key_column_usage k,
	...
	*/
	function UpdateBlob($table,$column,$val,$where,$blobtype='BLOB')
	{
		...
		if (strtoupper($blobtype) == 'CLOB') {
			$query = "UPDATE $table SET $column='" . $val . "' WHERE $where";
			return $this->Execute($sql) != false;
		}
		$query = "UPDATE $table SET $column=0x".bin2hex($val)." WHERE $where";
		return $this->Execute($sql) != false;
	}
	...
	function _query($sql,$inputarr)
	{
		...
		if (is_array($inputarr)) {
			...
			if (!is_array($sql)) $query = $this->Prepare($sql);
			$params = '';
			$decl = '';
			$i = 0;
