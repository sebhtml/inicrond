...
*/
	...
	*/
	function &SelectLimit($sql,$nrows=-1,$offset=-1, $inputarr=false,$secs2cache=0)
	{
		...
		if ($this->firstrows) {
			if (strpos($sql,'/*+') !== false)
				$query = str_replace('/*+ ','/*+FIRST_ROWS ',$sql);
			else
		...
		if ($offset < $this->selectOffsetAlg1) {
			if ($nrows > 0) {	
				...
				if ($this->databaseType == 'oci8po') {
					...
					$query = "select * from (".$sql.") where rownum <= :adodb_offset";
				} 
	...
	*/
	...
	function UpdateBlob($table,$column,$val,$where,$blobtype='BLOB')
	{
		...
		if ($this->databaseType == 'oci8po') 
			$query = "UPDATE $table set $column=EMPTY_{$blobtype}() WHERE $where RETURNING $column INTO ?";
		else 
			$query = "UPDATE $table set $column=EMPTY_{$blobtype}() WHERE $where RETURNING $column INTO :blob";
