...
class ADODB_ibase extends ADOConnection {
	...
	{	
		...
		$query = 'SELECT S.RDB$FIELD_NAME AFIELDNAME
	...
	function &MetaIndexes ($table, $primary = FALSE, $owner=false)
	{
        ...
        $table = strtoupper($table);
        $query = "SELECT * FROM RDB\$INDICES WHERE RDB\$RELATION_NAME = '".$table."'";
        if (!$primary) {
		...
		while ($row = $rs->FetchRow()) {
			...
			$query = "SELECT * FROM RDB\$INDEX_SEGMENTS WHERE RDB\$INDEX_NAME = '".$name."' ORDER BY RDB\$FIELD_POSITION ASC";
