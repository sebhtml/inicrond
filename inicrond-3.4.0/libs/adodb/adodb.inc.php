...
*/
	...
	*/
	...
	function CacheGetCol($secs, $query = false, $inputarr = false,$trim=false)
	{
	  	$rv = false;
	...
	* @return		the recordset ($rs->databaseType == 'array')
	...
	{	
		if (!is_numeric($secs2cache)) {
			if ($query === false) $query = -1;
			if ($offset == -1) $offset = false;
									  // sql,	nrows, offset,inputarr
			$rs =& $this->SelectLimit($secs2cache,$sql,$nrows,$offset,$this->cacheSecs);
		} else {
			if ($query === false) ADOConnection::outp( "Warning: \$sql missing from CacheSelectLimit()");
	...
	* If $query == false, then we purge all files in the cache.
 	*/
	...
	* then we can scale to 12.8 million unique cached recordsets. Wow!
	...
	function& AutoExecute($table, $fields_values, $mode = 'INSERT', $where = FALSE, $forceUpdate=true, $magicq=false) 
	{
		...
		case 'UPDATE':
		case '2':
			$query = $this->GetUpdateSQL($rs, $fields_values, $forceUpdate, $magicq);
			break;
		case 'INSERT':
		case '1':
			$query = $this->GetInsertSQL($rs, $fields_values, $magicq);
			break;
		default:
